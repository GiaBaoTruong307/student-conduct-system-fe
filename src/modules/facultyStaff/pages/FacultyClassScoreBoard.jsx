...
import { FACULTY_CLASSES } from "../constants/facultyStaff.constants";
...
// Tính tổng điểm GVCN từ savedScores, fallback về selfScores per-item
...
const FacultyClassScoreBoard = () => {
  ...
  const getGvcnScore = (member, idx) => {
    ...
    if (member.isLinkedToStudent) {
      ...
      // Nếu có savedScores hoặc selfScores, tính tổng đúng
      ...
      // Fallback: nếu GVCN đã lưu total
    ...
    // Non-linked: ưu tiên data đã lưu
    if (data?.total !== undefined && data.total > 0) return data.total;
  ...
  const handleViewDetail = (mssv) => {
    navigate(
      `/faculty-staff/bang-diem-khoa/${classId}/student/${mssv}?yearId=${selectedYearId}&semId=${selectedSemesterId}`
    );
  };
  ...
  if (!classInfo) {
    return (
      <div className="flex flex-col items-center justify-center h-48 gap-4">
        ...
        <button
          onClick={() => navigate("/faculty-staff/bang-diem-khoa")}
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 px-5 py-3 flex items-center gap-3">
        ...
        <span className="text-gray-600 text-sm">
          GVCN: <span className="font-medium text-gray-800">{classInfo.gvcn}</span>
        </span>
      </div>
...
export default FacultyClassScoreBoard;